  ...
  if (item) {
    ...
    // check for fields collision
      ...
      if (common.length && DeepDiff(_.pick(item.doc, common), _.pick(doc, common))) {
        debug('Redis-Oplog: Handling potential race condition on ' + uid + ': ' + common.join(', '));
    ...
    // either we have field collision or the doc was removed
      ...
      debug('Redis-Oplog: Handling potential race condition on ' + uid + (fields ? ': ' + fields.join(', ') : ''));
...
export const onTimer = (uid) => {
  ...
  if (item) {
    ...
    if (!collection) {
      ...
      if (driver && collection._readPreference !== 'primary') collection = new Mongo.Collection(item.collectionName, {
        _driver: driver,
    ...
    if (!collection) {
      ...
      //console.log('masterCollections', masterCollections)
    }
