...
import { dispatchInsert, dispatchRemove, dispatchUpdate } from './lib/dispatchers';
...
export default class Mutator {
  ...
  static update(Originals, selector, modifier, _config, callback) {
    ...
    const [config, queryConfig] = getMutationConfig(this, _config);
    ...
    try {
      ...
      const result = config.skipDB ? 0 : Originals.update.call(this, updateSelector, modifier, queryConfig);

      // getting fields to $unset
      const cleared = modifier.$unset && _.keys(modifier.$unset) || [];
      if (modifier.$set) _.each(modifier.$set, (value, key) => {
        if (value == null) cleared.push(key);
      });

      const { topLevelFields } = getFields(modifier);
      // Build matcher for positional queries
      ...
      if (this._skipDiff || config.skipDiff) {
        ...
        docs.forEach((doc, index) => {
          // Match positional query for modification
          const queryResult = matcher.documentMatches(doc);
          LocalCollection._modify(doc, modifier, { arrayIndices: queryResult.arrayIndices });
        ...
        docs.forEach((before, index) => {
          ...
          const queryResult = matcher.documentMatches(before);
          LocalCollection._modify(after, modifier, { arrayIndices: queryResult.arrayIndices });
