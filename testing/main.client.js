  ...
  describe('It should work with: ' + key, function () {
    it('Should detect a removal', function (done) {
      ...
      waitForHandleToBeReady(handle).then(() => {
        createSync({ game: 'chess', title: randomTitle }).then((id) => _id = id);
      });
    });
    ...
    it
    ('Should be able to detect subsequent updates for direct processing with _ids', function (done) {
      createSync([
    ...
    it('Should work with the $pull and $set in combination', function (done) {
      createSync({
        ...
        waitForHandleToBeReady(handle).then(() => {
          ...
          update(_id, {
            $pull: {
              // This won't work as it is not supported by Minimongo at all
              // connections: { $in: [1] },
              connections: 1,
            },
            $set: {
    ...
    it('Should work properly with limit-sort kind of queries', function (done) {
      ...
      removeSync({ context }).then(() => {
        ...
        ]).then(([_id1, _id2, _id3, _id4, _id5, _id6]) => {
          const handle = subscribe(
            {
              context,
            },
    ...
    it('Should work with deep nest specified fields', function (done) {
      ...
      createSync({ context, passengers: [], }).then((_id) => {
        const handle = subscribe(_id, {
          fields: {
            context: 1,
            'passengers.name': 1,
    ...
    it.skip('Should detect 3rd level nesting changes', function (done) {
      const context = 'deep-level-nesting-' + Random.id();

      const handle = subscribe({ context });

      waitForHandleToBeReady(handle).then(() => {
        // TODO: For some reason this test gets assertion error because of 2 times changed event?
        const cursor = Collection.find({ context });
        cursor.observeChanges({
          changed(docId, doc) {
            assert.isObject(doc.item);
            assert.equal(doc.item.profile.name, 'Elena Smith');
            done();
          },
        });
